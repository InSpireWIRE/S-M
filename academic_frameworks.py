...
class AcademicFrameworkAnalyzer:
    ...
    def _analyze_documentary_mode(self, synthesis: str, data: Dict) -> Optional[str]:
        """Apply Nichols' Documentary Modes analysis"""
        
        prompt = f"""
        Based on this documentary conversation and synthesis, analyze which of Nichols' documentary modes 
        best fits this project and suggest strategic mode mixing.
        
        SYNTHESIS: {synthesis}
        
        CONVERSATION DATA: {self._format_conversation(data)}
        
        The 6 Documentary Modes to consider:
        - POETIC: Visual/emotional associations over linear narrative
        - EXPOSITORY: Direct argument with authoritative narration
        - OBSERVATIONAL: Fly-on-the-wall, subjects speak for themselves
        - PARTICIPATORY: Filmmaker actively engages with subjects
        - REFLEXIVE: Acknowledges the construction of documentary itself
        - PERFORMATIVE: Filmmaker's personal journey intertwined with subject
        
        Provide 2-3 specific suggestions about:
        1. Which mode(s) naturally fit their material
        2. Where mixing modes could strengthen specific scenes
        3. Mode-specific techniques that serve their story goals
        
        Frame suggestions conversationally:
        - "Your material naturally lends itself to..."
        - "Consider shifting to participatory mode when..."
        - "An observational approach might reveal..."
        
        Start with: [Framework: Documentary Modes - Bill Nichols]
Immediately follow with: (Based on Bill Nichols' theoretical framework)
        
        Limit response to 150 words. Be specific to their actual story.
        """
        ...
        try:
            response = openai.ChatCompletion.create(
                model="gpt-3.5-turbo-16k",
                messages=[
                    {"role": "system", "content": "You are a documentary consultant analyzing which filmmaking modes best serve the story. Be practical and specific."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.7,
                max_tokens=300
            ...
            logging.error(f"Documentary mode analysis failed: {e}")
            return None
