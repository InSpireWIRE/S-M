...
class ConversationManager:
    ...
    def save_turn(self, conversation_id: str, question: str, answer: str) -> Dict:
        ...
        # LIMIT CONVERSATION DEPTH - Stop after 5 turns
        if current_turn + 1 >= 5:  # Stop when we're about to create turn 5
            return {
                'turn_id': None,
                'saved': True,
                'next_questions': [],
                'complete': True,
                'message': 'Great work! You\'ve provided enough context for a detailed analysis.',
        ...
        next_questions = self.generate_followup_questions(answer, context_for_followup)
    ...
    def generate_followup_questions(self, answer: str, conversation_context: Dict) -> List[str]:
        ...
        try:
            ...
            get_followup_for_gap = None
        
        # Map questions to gap types
        question_map = {
            "stopping your characters": "missing_conflict",
            "actually changes": "no_transformation",
            "anyone care": "no_stakes",
            "access": "unclear_access"
        }
        ...
        # If we have a gap type and the function exists, use specific follow-ups
        if gap_type and get_followup_for_gap:
            return get_followup_for_gap(gap_type, 1)
        
        # Analyze what they said for fallback questions
        doc = self.nlp(answer)
        ...
        if people:
            return [
                f"OK but what makes {people[0]} worth following for months?",
                f"What's {people[0]} risking by letting you film?",
                f"How long did it take {people[0]} to trust you?"
            ]
